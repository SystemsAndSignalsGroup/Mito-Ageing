...
parser = argparse.ArgumentParser(description="single cell RNAseq mixed genotype clustering using sparse mixture model clustering with tensorflow.")
...
parser.add_argument("-s", "--seq_type", required = True, help = "Sequencing type either 10x or custom")
...
parser.add_argument("--skip_freebayes", required = False, default = "FALSE",
    help = "don't variant call with freebayes (This is left false as a default)")
...
if args.skip_freebayes == "TRUE":
    args.skip_freebayes = True
else:
    args.skip_freebayes = False
    ...
    while any_running:
        ...
        for (index, region) in enumerate(regions):
            ...
            if not block:
                ...
                if args.seq_type == "custom":
                    p = subprocess.Popen(["custom_renamer.py", "--bam", args.bam, "--barcodes", args.barcodes, "--out", fq_name,
                        "--chrom", chrom, "--start", str(start), "--end", str(end), "--no_umi", str(args.no_umi),
                        "--umi_tag", args.umi_tag, "--cell_tag", args.cell_tag])
...
def retag(args, minimap_tmp_files):
    ...
    final_bam = args.out_dir + "/minimap_tagged_sorted.bam"
...
def freebayes(args, bam, fasta):
    ...
    with open(args.out_dir + "/bcftools.err", 'w') as vcferr:
        with open(args.out_dir + "/merged_sorted_vcf.vcf", 'w') as vcfout:
            subprocess.check_call(['bcftools', 'sort', args.out_dir + "/souporcell_merged_vcf.vcf", "-T", args.out_dir + "/temporary"], stdout = vcfout, stderr = vcferr)
    if not args.common_variants == None:
        ...
        subprocess.check_call(['mv', args.out_dir + "/vcftmp", args.out_dir + "/merged_sorted_vcf.vcf"])
    subprocess.check_call(['rm', args.out_dir + '/souporcell_merged_vcf.vcf'])
    subprocess.check_call(['bgzip', args.out_dir + "/merged_sorted_vcf.vcf"])
    final_vcf = args.out_dir + "/merged_sorted_vcf.vcf.gz"
...
#### MAIN RUN SCRIPT - Modified to only run remap and variant calling sequences
...
if not args.skip_remap:
    ...
    bam = args.out_dir + "/minimap_tagged_sorted.bam"
else:
    bam = args.bam
if not args.skip_freebayes:
