...
#THIS rename.py has been modified to work with UMI_TOOLS data
...
with open(args.out,'w') as fastq:
    for (index,read) in enumerate(bam):
        ...
        readname = read.qname
        readname_prefix = readname.split("_")[0]
        if read.has_tag(CELL_TAG) and read.get_tag(CELL_TAG) in cell_barcodes:
            if args.no_umi:
                fastq.write("@"+readname_prefix+";"+cell_barcode+"\n")
            else:
                fastq.write("@"+readname_prefix+";"+cell_barcode+";"+UMI+"\n")
